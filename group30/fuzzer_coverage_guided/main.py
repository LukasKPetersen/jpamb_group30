...
class CoverageGuidedStrategy(Strategy):
    ...
    def mutate_input(self, mutation_input: Input) -> Input:
        mutated_inputs = []

        for input_values in mutation_input.values:
            match input_values.type:
                case jvm.Int():
                    # Just doing one form of mutation to input
                    v = input_values.value
                ...
                case _:
                    raise NotImplementedError(f"Mutation not implemented for type: {input_values}")
